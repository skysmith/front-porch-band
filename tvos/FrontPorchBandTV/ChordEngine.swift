...
enum ChordEngine {
    ...
    private static func parseChordToken(_ token: String) -> (root: String, suffix: String)? {
        ...
        guard
            let match = chordRootRegex.firstMatch(in: token, range: range),
    ...
    private static func swapEnharmonicRoot(_ chordName: String) -> String {
        ...
        guard let parsed = parseChordToken(chordName), let replacement = equivalents[parsed.root] else {
            return chordName
        }

        return replacement + parsed.suffix
    }
    ...
    private static func buildRelaxedCandidates(_ chordName: String) -> [String] {
        ...
        func append(_ suffix: String) {
            let candidate = parsed.root + suffix
            guard candidate != chordName, !relaxed.contains(candidate) else {
                return
            }
            relaxed.append(candidate)

            let enharmonic = swapEnharmonicRoot(candidate)
            if enharmonic != candidate && !relaxed.contains(enharmonic) {
                relaxed.append(enharmonic)
            }
        }

        if parsed.suffix.hasSuffix("maj7") { append("") }
        if parsed.suffix.hasSuffix("m7") { append("m") }
        if parsed.suffix.hasSuffix("7") { append("") }
        if parsed.suffix.hasSuffix("add9") { append("") }
        if parsed.suffix.hasSuffix("6") { append("") }
        if parsed.suffix.hasSuffix("sus2") || parsed.suffix.hasSuffix("sus4") { append("") }
